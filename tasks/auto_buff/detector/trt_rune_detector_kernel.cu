...
__global__ void rune_detector_preprocess_kernel(
  ...
  int padding_x, int padding_y)
{
  ...
  const float src_x = (static_cast<float>(x) - padding_x) / scale;
  const float src_y = (static_cast<float>(y) - padding_y) / scale;
...
extern "C" void launch_rune_detector_preprocess(
  const unsigned char * src, int src_width, int src_height, std::size_t src_step,
  float * dst, int dst_width, int dst_height, float scale, int padding_x, int padding_y,
  cudaStream_t stream)
{
  ...
  rune_detector_preprocess_kernel<<<grid, block, 0, stream>>>(
