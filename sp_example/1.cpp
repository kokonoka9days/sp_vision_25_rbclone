...
#include "tasks/auto_aim/aiming/commandgener.hpp"
#include "tasks/auto_aim/detection/multithread/mt_detector.hpp"
  ...
  while (!exiter.exit()) {
   ...
   /// 全向感知逻辑
    if (tracker.state() == "lost")
      io::VisionToGimbal all_gimbal = decider.decide_g(yolo, gimbal_pos, usbcam1, usbcam2, back_camera);
    else
     gimbal = aimer.aim(targets, t, gs.bullet_count);

    /// 发射逻辑
    bool should_shoot = shooter.shoot_g(all_gimbal, aimer, targets, gimbal_pos);

    gimbal.send(all_gimbal);
