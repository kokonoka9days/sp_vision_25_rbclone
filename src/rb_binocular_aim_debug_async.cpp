...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    if (targets.empty()) {
      ...
      const Eigen::Vector3d center_world = target.center();
      const Eigen::Vector3d velocity = target.velocity();
      ...
      for (const auto & pose : target.armor_pose_list()) {
        const auto image_points = frame_solver.reproject_pose(pose, target.armor_type);
